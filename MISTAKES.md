...
| `[purity-leak]` | 3 | **SHIPPED**: `make purity-check` (`tests/purity_engine_scan.py --check` + the `win_witness_guard` tests over `WIN_WITNESS_LEDGER`), a `make test` prerequisite; the provenance registry `scripts/provenance_check.py` is SHIPPED but runs under `make provenance-check`, which `make test` does not call |
| `[inert-treatment]` | **8** | **PROMOTED 2026-08-28** (project instruction file); **partial**: `scripts/check_mechanism_receipt.py` VOIDs an armed mechanism whose counter never moves, `scripts/redo_arm_gate.py` + `_REDO_ARM_DEADLINE_ITERS` kill an armed-but-never-firing run at iter 40 (raised from 25 on 2026-08-27, `989ad38`, for the tau=0.10 surgical operating point); blind to a mechanism nothing imports, to one armed at a reachable-but-wrong dose, and, most recently, to an instrument a registration ADOPTED and never wrote at all, which leaves the same receipt as one that ran and found nothing |
...
| `[overlapping-fixes]` | 1 | — (new category 2026-09-03: two independently-prepared patches target the same file with contradictory designs; neither producing agent can detect it in isolation) |
...
## 2026-09-03 — [overlapping-fixes] Two independently-prepared fixes to the same purity subsystem took contradictory approaches, and only landing them in the wrong order would have exposed it

- **What happened:** An orchestrator dispatched two separate pieces of work in parallel: a purity-checker staleness fix (design the scanner to tell "gitignored data, code site still real" apart from "site actually removed," keeping four disclosure rows for `runs/clear_recensus/contra/probe_*.py`) and a batch-gate measurement (`claims-readme-commands`, prepared 2026-09-02, whose only surviving hunk deletes those same four disclosure rows outright, on the premise that they were simply wrong entries). Both were independently measured green in scratch. The staleness fix landed first (`82b2c84`). Only a manual re-read of the batch's diff, done because its patch touched `docs/purity/` and the orchestrator recognized the file, caught that the batch would delete the exact rows the just-landed commit was written to keep and correctly classify. The batch was held rather than landed.
- **Root cause:** Neither producing agent knew the other piece of work existed; each measured its own patch against a clean baseline and confirmed correctness in isolation, which neither could have detected. The batch-gate measurement report's own notes even flagged the failing tests it produced as "different pre-existing bugs, unrelated to this patch" — true in isolation, but the two pieces of work were not actually unrelated, they were two answers to the same open question sitting in two different session-scratch queues.
- **Consequence:** No wrong commit landed. Caught before landing, not after; cost was one extra manual read of a diff before dispatching a land pass, not a revert. Had the batch landed first, the staleness fix's own tests would have had nothing to exercise on a fresh clone (the rows it teaches the checker to classify would already be gone), and the contradiction would have surfaced later as confusion about why the new UNVERIFIABLE code path was untested, not as an obvious landing-time conflict.
- **Rule (draft):** Before dispatching two pieces of prep work that can land independently, check whether their target files overlap; if they do, read both designs against each other before either lands, not just each against its own baseline. A patch's own "unrelated to this batch" note about a shared file is a claim from one side only and does not settle the question.

## 2026-09-03 — [process] A scratch venv copy silently wrote through broken shebangs to the shared real venv, corrupting a live measurement in progress

- **What happened:** An agent building an isolated pytest-version comparison ran `cp -R` to copy the repo's shared `.venv` into scratch, then ran `./copied_venv/bin/pip install ...` against the copy. The copy's console-script shebangs (`bin/pip`, `bin/pytest`) still pointed at the absolute path of the original `.venv`, so both install calls landed on the real, shared venv instead of the copy: pytest went from the pinned 9.0.3 to 9.1.1. Two full-suite pytest processes were running against that same shared venv at the time (one of them a batch-gate measurement), and both ran to completion under the wrong interpreter version before anyone noticed. The agent tried to self-correct (`pip install --force-reinstall pytest==9.0.3`) but the permission classifier correctly blocked it as a write to a path it read as read-only, and the agent stopped and reported rather than working around the block — the right call. A second agent independently verified the report (checked the live version, the running PIDs, the log file) before acting, then attempted the same fix and hit the identical classifier block; the fix needed a human to run it directly.
- **Root cause:** `cp -R` on a Python venv copies bytes, not the venv's own relocation logic — console-script shebangs are written as absolute paths at `venv create`/`pip install` time and a raw copy does not rewrite them, so every entry point in the copy silently retargets the original. This is a known venv footgun with no guard anywhere in this session's scratch tooling.
- **Consequence:** Two in-flight measurements ran under the wrong pytest version for an unknown window before the fix landed; both were later re-measured after the fix rather than trusted as-is, and one batch's failure counts genuinely shifted between the two measurement passes in ways consistent with the corrupted window, not with real regressions. No repo commit was affected; `.venv` is gitignored.
- **Rule (draft):** Never `cp -R` a venv into scratch expecting it to be self-contained. Either rebuild the venv fresh in scratch (`python -m venv` + reinstall from a lockfile) or use `--copies`-aware tooling that rewrites shebangs, and before any `pip install` inside a "scratch" venv, print `sys.prefix` and confirm it resolves inside the scratch path, not the source tree, before the install call.

---

## 2026-09-02 — [process] The day wave halted on a correct count: after DO-11's fix round the count gate expected 6203 + 1 (the fix round's one bash check) against a tree that read 6203 + 3 (the item's three collected tests)
- **What happened:** Day-wave item DO-11 (queue row 5 of `personal_os/reports/macos-emulation-and-training/2026-09-02-DAY_WAVE_QUEUE.md`: `scripts/git_safe.sh`, `scripts/worktree_census.py`, `tests/test_git_safe.sh`, `tests/test_worktree_census.py`) landed as `e92046b` at 04:50 on baseline `99d6f27` (6203 passed, `2026-09-01-outstanding/receipts/DO-7_receipt.log:128`); its receipt `2026-09-01-outstanding/receipts/DO-11_receipt.log:67` reads `6206 passed, 26 skipped, 40 deselected, 3 xfailed`, the 3 tests `tests/test_worktree_census.py` collects. The independent review returned findings; the fix round amended the commit to `f02f03e` (committer time 05:11), adding a 13th check to `tests/test_git_safe.sh` (bash, not pytest-collected) and zero collected tests, and re-ran the full gate: `6206 passed` again (receipt line 194). The wave script (`day_wave.js`, session scratch, line 76) then asserted `final.count === baseline + final.tests_added` where `final` is the fix round's return, whose `tests_added` was 1, and halted the wave: `count mismatch: reported 6206, expected 6203 + 1`. The queue row ("6165 + 3"), review correction 17 ("Gate is baseline + 3") and the commit body all say +3; `pytest --collect-only -q tests/test_worktree_census.py` reads `3 tests collected` at this closeout; the closeout's own foreground gate at `f02f03e` reads 6206 (receipt `2026-09-01-outstanding/receipts/closeout-2_receipt.log`). The tree was right and the assertion was wrong.
- **Root cause:** The wave's result schema carries one bare integer `tests_added` with no definition (collected by the full suite or any check at all; this return's delta or the item's whole delta), and the count gate reads it from whichever return came last (`let final = r`, then `final = f` after a fix round). A fix round that adds a bash check reports 1 in good faith; the gate then compares the suite's count to a number that was never about the suite. Three arithmetic claims existed for one measured number (the implementer's +3, the reviewer's request's 6218, the fix round's +1) and the script trusted the last one instead of re-deriving from the tree.
- **Consequence:** No wrong commit and nothing lost: `f02f03e` is on `main` with its receipt and its collect count in agreement, and the wave had landed all six rows above its cut line (0b `3445f73`, DO-16 `3bc7c36`, DO-10 `cba5e57`, DO-30 `e4379d3`, DO-7 `99d6f27`, DO-11 `f02f03e`; 6165 to 6206, 0 failed throughout). Cost: the wave stopped at 05:11 with about ten hours of window left and DO-25 next, on a false alarm whose message named a correct number as wrong, and the closeout spent its time proving a non-defect instead of the wave landing DO-25.
- **Rule (draft):** A count gate's expected delta is derived from the tree, never from an agent's return: `pytest --collect-only -q` on the commit's added and changed test files (or the plan row's own figure, which the reviewer checks against the collect), and a fix round that adds zero collected tests keeps the item's delta. When two claimed deltas disagree, re-derive; halting on either one is a guess.

---

## 2026-09-02 — [process] The day wave halted at its first row: a launchd engine tick started a full-suite pytest in this tree nine minutes after the wave launched, and the wave's only answer to a foreign pytest was to stop
- **What happened:** The day wave (`personal_os/reports/macos-emulation-and-training/2026-09-02-DAY_WAVE_PLAN.md`, queue `2026-09-02-DAY_WAVE_QUEUE.md`) launched at 01:37 on `a755d80`. At 01:46:11 the `com.stits.nes-engine` launchd agent (`~/Library/LaunchAgents/com.stits.nes-engine.plist`, `StartInterval` 600, `scripts/engine_driver.py --once`) launched its recurring `suite_check` action: `runs/engine/journal.jsonl` `t 1788338771`, attempt 42, pid 68246 running `scripts/run_suite_check.py --out runs/engine/suite_check.json --timeout 5400`, child pid 68247 running `pytest tests/ -q --timeout=180` (the whole suite, slow tests included, under a 1.75 h reaper). Its previous run had launched at 19:37:09 and been reaped at 20:28:34, so `RECURRING_COOLDOWN_H = 6.0` expired at 01:37, inside the wave's window. STEP-0's baseline gate (`2026-09-01-outstanding/receipts/STEP0_baseline.log`: 6165 passed, 0 failed, 26 skipped, 40 deselected, 3 xfailed in 496.19 s) ran 01:43:54 to 01:52:11 and overlapped the engine's run for six minutes; its `pgrep` for live pytest was taken once, before 01:46, and not re-taken before the gate. Row 0b (the block-runner root-state sandbox, the one row that could lift the live-protocol NO-GO) started at 02:03, applied its diff to `src/forge/block.py` and `tests/test_forge_block.py`, found pid 68247 live, honored "never run two pytest suites at once", never started its own gate, restored both files at 02:16:57 (`git status --porcelain` empty at closeout) and returned halted about 14 minutes into a 90-minute box. Nothing landed; HEAD is still `a755d80`; every `entrance_after_2.state` under `runs/` still carries its July or August mtime.
- **Root cause:** Two schedulers own this tree and this `.venv` with nothing shared between them: the engine's `plan()` holds an emulator action back only when `emulator_busy()` sees a solver or campaign name in `ps`, and `suite_check` is `needs_emulator=False`, so it checks nothing before launching a pytest, and the wave's plan (rulings 11 and 15) checked for a live pytest once at STEP-0 instead of disarming the engine or taking a lock the engine reads. The wave's rule for a foreign pytest was absolute (halt) where the contention was transient (a bounded run with a known reaper), so the implementer had no sanctioned move short of stopping the wave.
- **Consequence:** No wrong claim banked and no commit. Zero of the twenty queued rows were attempted; the live Forge protocol stays NO-GO (Ruling 13) for a reason unrelated to the runner. Cost: the wave's window from 02:03 to closeout, and a STEP-0 baseline that was measured beside a concurrent full suite. Its count equals the landing receipt's 6165 to the test, so it stands, but that overlap is the exact condition the rule exists to prevent, and STEP-0 did not see it.
- **Rule (draft):** An unattended wave in this tree starts by unloading `com.stits.nes-engine` (`launchctl bootout gui/<uid>/com.stits.nes-engine`) and its closeout re-arms it, with both actions in the wave's receipts; and a gate that finds a foreign `pytest tests` live waits for that pid with a bounded timeout (the foreign run's own `--timeout` plus margin), records the wait, and re-checks `pgrep` immediately before launching its own run. Halting is for a writer, not for a reader that will finish on its own.

---

## 2026-09-02 — [vacuous-gate] Piece (f)'s real-child positive control read PASS on a receipt that could not tell a live watchdog cycle from a pre-launch refusal
- **What happened:** The Forge block runner landed at `8986633` with its commit body and progress row reading "Section 4 row (f) PASS (not PASS-SYNTHETIC)" on a real-child receipt (`personal_os/reports/macos-emulation-and-training/2026-09-02-forge/receipts/f_receipt.log`, run 2026-09-02T06:24:19Z, cv_hall, `inject_wrongful_reset:true`, `watchdog_trips:1`, `caught:true`, `banked:[]`). A read-only skeptic (`f_real_child_skeptic.md`, same directory) showed `started == ended` in the same second and `run_lock_hours 0.0`: the trip fired on a root-state sha computed once before launch and compared on the first poll, before the child had executed a line of its own code; `inject_wrongful_reset` was copied into the receipt and never acted on; `positive_control.caught` was set on any abort. The amended commit `e80c123` fixed all four (the injection now corrupts the child's own root file after the first real progress row; the sha is re-read every poll; `caught = injected`; `aborted` is set on the trip path) and re-stated the gate as PASS-SYNTHETIC-plus-launch-smoke, which is the specification's VOID column for row (f): synthetic control passes, live blocks do not run. The corrected live-child run is still not in the tree. `e80c123`'s injection appends bytes to whatever file `--root-state` names, and against a real plan that is a shared root under `runs/`; the per-block copy that makes the injection safe is drafted (`row0b_runner_correction.md`, same directory: live child 63 s, two progress rows before injection, shared sha unchanged) and not landed. The landing commit that carries this entry lands the two FORGE-GRANT entries with that gate at VOID, each stating that nothing starts under it until the live-child receipt exists.
- **Root cause:** The positive control's receipt fields were derived from the abort path, not from the injection path: `caught` meant "aborted while the flag was set", and nothing in the receipt asserted that a progress row had been parsed or that `ended > started` before the trip counted. The gate could not say no to a child that died at startup, so the first run that did exactly that read as a pass, and the piece's own return value relayed it as PASS.
- **Consequence:** The live half of the night's plan did not run: no block, no cycle, both grants landed with "no block has run under it" and a start condition still unmet (Ruling 13, NO-GO). No artifact was banked and no shared root state was modified: all five `entrance_after_2.state` copies under `runs/` carry their July and August mtimes, and `runs/cv_chain_poweron`'s sha256 prefix `5a6c5ea3a376a917` equals the skeptic's pre-injection reading. Cost: one skeptic pass, one fix round, and the two live cycles the window had budgeted.
- **Rule (draft):** A positive control's receipt carries the evidence that its trigger was reached inside the live loop, not only that the loop ended: at least one telemetry row parsed before the trip, `ended > started`, and `caught` derived from the injection path. A control that fires before the subject under test has run reports UNMEASURED, not caught.

---

## 2026-09-01 — [unverified-claim] The reward sweep named three banked LEARNED rates as trained under a contaminated setting, having matched configs by start-state filename instead of reading the run manifests
- **What happened:** The reward non-farmability sweep (`personal_os/reports/macos-emulation-and-training/2026-09-01-outstanding/reward-sweep.md`) concluded that three of the four banked LEARNED numbers (1-2 38/100 and 31/100, 1-3 21/100, 1-4 51/100) "were trained under `checkpoint_scale: 1.0`", the un-clawed dense x-ladder bonus, and flagged the 1-1 43/100 as unpinned across six candidate configs "all 1.0". The queued follow-up (DECIDE-12) was to write that sentence onto `CLAIMS.md` as a dated annotation on the flagship scoreboard. It is false in every part. Each run records the profile it trained under, and reading them settles it: `runs/consol2/manifest.json` `config.base_profile` names `configs/mario_1_2_consol2.yaml:116` (`checkpoint_scale: 0.0`); `runs/consol2_1_3/manifest.json` and `runs/consol2_1_3_round2/manifest.json` name `configs/mario_1_3_online_v1.yaml:101` (`0.0`); `runs/online_1_4/manifest.json` names `configs/mario_1_4_online_v1.yaml:101` (`0.0`); `runs/interference/manifest.json` records the 1-1 specialist at `config.specialists["1-1"].profile`, which is `configs/mario_1_1_backward.yaml:109` (`0.0`). Every `phase_configs/*.yaml` those runs recorded sets `0.0`, and the four `metrics.jsonl` (326, 30, 109 and 120 rows) log `reward_forward`, `reward_completion` and `reward_time_penalty` in every row and `reward_checkpoint` in none. The judged pass that caught it is `2026-09-01-outstanding/reward-sweep-JUDGED.md` (sections 2, 7 and 8).
- **Root cause:** The sweep inferred which training config produced each banked checkpoint from the shape of the start-state filenames it saw, and the 22 configs that share that filename shape do enable the ladder. Nothing in the sweep opened the artifact that actually records the answer. A run manifest is written by the trainer at launch and names its own profile and phase configs; a config chosen by resemblance is a guess wearing a path. The sweep also had no way to be contradicted by the training logs, because it never read them: the same `metrics.jsonl` files that would have shown `reward_checkpoint` paying were on disk the whole time.
- **Consequence:** Nothing was banked and no public number moved. The cost is one judge pass to overturn it, plus the near miss: the annotation would have put a false sentence about training provenance on the ledger that carries all four banked rates, where it would have propagated into the README and every summary downstream of it. The eval-side numbers were never in question; only the story about how they were trained.
- **Rule (draft):** A claim that a banked number was trained under a given setting cites the run manifest and the recorded phase config, never a config chosen by name or by filename resemblance; and where the claim is load-bearing enough to enter a ledger, it ships as a script that re-derives the chain (here `scripts/check_learned_lineage.py`, `make lineage-check`), not as a sentence.
...
## 2026-09-01 — [process] `main` has depended on uncommitted working-tree code since 2026-08-26: a committed gate test imports a function that was never committed
- **What happened:** `tests/test_anti_vacuity_gates.py` (committed in `eb77a0a`, 2026-08-26, "a gate must prove it can fail") imports `behavioural_lives_verdict` from `scripts/discover_observables.py`. That function exists only in the working-tree edits to `discover_observables.py` (+739 lines, mtime 2026-08-26 08:58) that were never committed. On a clean checkout of `main` at any commit from `eb77a0a` through `05f1824`, the suite fails with an ImportError and collects 34 fewer tests (30 in the untracked `tests/test_lives_behaviour_gate.py`, 5 in the uncommitted `tests/test_onboard_game.py` edits). The 2026-09-01 baseline of 6,061 passed only because the uncommitted files were present in the operator's tree; parking them on `wip/onboarding-gate-2026-08-26` exposed it.
- **Root cause:** The anti-vacuity census committed the *test* half of a gate the same morning its *implementation* half was edited and left uncommitted; nothing checks that `main`'s test suite passes from a clean checkout, so a tree that passed locally masked a `main` that could not.
- **Consequence:** Six days in which anyone cloning `main` would see a red suite on the very test that guards against vacuous gates; no wrong claim banked. Found by the 2026-09-01 execution of the master list, which moved the WIP to a branch and re-ran the suite. Landed on `main` the same day after validating the gate on all eleven fanout ROMs driven fresh (`reports/…/2026-09-01-lives-gate-validation.md`): 8 agree with the adjudicated fanout (exact numeric matches on Bad Dudes and Journey to Silius), 3 disagree — one is the doc's own error (Mega Man 3, regressed by its smoke), two are gate gaps that stay open: no independent root>0 re-check (Kid Icarus can pass on a start-zero idle-quiet byte), and probe arms of 320–700 steps too short for a ~117-step free-runner (Paperboy). The gate is correct on the three headline failure modes and not yet safe to trust unattended.
- **Rule (draft):** A commit that adds or changes a test is run against a clean checkout of the branch (\`git stash\`-free: a temporary worktree of HEAD), not only against the working tree; the receipt for "suite green" names the commit it was measured on, not the tree.
...
- **Consequence:** No data lost, by luck of the domain. `a0a7664` extracted the lock into `src/utils/run_lock.py` with a CLI wrapper; at ship time it guarded two writers. The remaining writers (`go_explore_chain.py`, `go_explore_solve.py`, `run_v31_eval_ladder.py`, `run_online_campaign.py`, `merge_recovery_ladder.py`, `watch_asm.py`, `discover_observables.py`) are still unguarded as of this entry, and nothing locks git refs.
  This commit ("locks: run_lock on every solver-chain writer (6 scripts), tmp+replace for discover_observables selftest") wires the lock into six of those seven — `go_explore_chain.py`, `go_explore_solve.py`, `run_v31_eval_ladder.py`, `run_online_campaign.py`, `merge_recovery_ladder.py`, and `discover_observables.py`'s `_selftest()` (also moved off truncate-in-place `write_text` onto tmp-file + `os.replace()`) — leaving `watch_asm.py` alone on inspection: it is a read-only PyQt6 ROM viewer with no write to any shared path, so the seventh name on this list turned out to be miscounted; git refs still aren't locked.
  Every per-path lock above only stops two instances of the SAME script colliding on the SAME `--out`; it said nothing about a DIFFERENT script stepping the same physical emulator pool from an unrelated `--out` at the same time — `scripts/collect_substrate_pairs.py`'s docstring named that exact gap in prose ("the operator gate is upstream — do not run while anything else steps the pool") with nothing enforcing it. This commit ("locks: resource-keyed emulator-pool lock replaces the 'do not run while anything else steps the pool' docstring") adds `acquire_resource`/`release_resource` to `src/utils/run_lock.py` (a lockfile keyed by a shared resource NAME, not a path any one script owns) and wires it into the four writers that step the pool directly — `collect_substrate_pairs.py`, `interference_falsifier.py`, `eval_shared_substrate.py`, and `go_explore_solve.py`'s `Solver.__init__` — turning that docstring's prose into a call.
  A parallel gap on the same list: only `run_online_campaign.py` and `train_game.py` refused to START below the free-disk floor — `go_explore_chain.py` ("machine-busy and hands-off") and `night2_runner.py` ran hours-to-days with no floor at all. This commit ("durability: disk floor on go_explore_chain and night2_runner via src/utils/disk_floor.py") extracts `run_online_campaign.py`'s `disk_floor_breach` into `src/utils/disk_floor.py` (same refuse-below-40GB / fail-open-on-stat-error semantics, plus an `allow_low_disk` override) and wires it into `go_explore_chain.py` and `night2_runner.py` at launch, closing that pair.
