//! ASM-CPU vs Rust-slow-CPU lockstep on real Castlevania II - Simon's
//! Quest (MMC1, mapper 1).
//!
//! Mapper 1 exposes a `prg_asm_window` (rebuilt on the shift-register
//! bank writes at $8000-$FFFF), which routes MMC1 games onto the
...
//!
//! `tests/parity/test_mesen_lockstep.py` records +3 bytes of extra
//! RAM divergence vs the Mesen oracle with `asm_cpu` on for this ROM
//! over 120 idle frames — this test is expected to surface that as a
...
const CASTLEVANIA2: &str = "../roms/Castlevania II - Simon's Quest (USA).nes";
...
#[test]
fn find_first_asm_divergence_castlevania2() {
    lockstep(CASTLEVANIA2);
}
