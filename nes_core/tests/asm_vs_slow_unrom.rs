//! ASM-CPU vs Rust-slow-CPU lockstep on real Castlevania (UNROM,
//! mapper 2).
//!
//! Mapper 2 exposes a `prg_asm_window` (rebuilt on the $8000-$FFFF
//! bank-select writes; UNROM's fixed top bank plus one switchable 16
//! KB bank), which routes UNROM games onto the AArch64 ASM fast path.
//! This test proves the ASM path is byte-exact against the
//! interpreter on the real ROM: it lockstep-runs the bulk path
//! (`nes.step`, ASM + multi-instruction batches) against the slow
//! path (`nes.tick` loop) from cold boot through the title/attract
//! sequence, comparing CPU registers + cycle count after every
//! instruction and full 2 KB system RAM at a regular stride.
//!
//! `tests/parity/test_mesen_lockstep.py` records +1 byte of extra RAM
//! divergence vs the Mesen oracle with `asm_cpu` on for this ROM over
//! 120 idle frames — this test is expected to surface that as a
//! concrete first-divergence site.
...
const CASTLEVANIA: &str = "../roms/Castlevania (USA) (Rev A).nes";
...
#[test]
fn find_first_asm_divergence_castlevania() {
    lockstep(CASTLEVANIA);
}
