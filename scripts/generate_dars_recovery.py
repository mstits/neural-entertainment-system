"""DARS — Disturbance-Augmented Re-Search recovery dataset (DART core).

Builds a noise-augmented imitation dataset so the student learns to RECOVER from
the exact off-equilibrium (drifted) states 25%-sticky evaluation will push it
into — the covariate-shift fix from the research brief.

Mechanism (DART, Laskey et al. 2017, adapted honestly to our search teacher):
for N samples, restore a random state s_k along a verified Go-Explore solution,
inject d~U(1,5) frames of 25% sticky-action noise to reach a DRIFTED state, then
have the EXPERT relabel the drifted rollout — step the solution's actions from
the time-resynced index for a recovery window, capturing (drifted_obs,
expert_action) pairs. Keep a sample only if the relabeled rollout SURVIVES the
window and makes forward progress toward the goal (wavefront distance decreases),
so the dataset contains genuine recoveries, not doomed drifts. States the expert
can't recover (the precise obstacles) are intentionally dropped — PPO + the
wavefront reward own those.

Output: a single .npz (obs_0, act_0) — same format replay_to_demos.py emits, so
it drops straight into the trainer's demo-anchor (DQfD) after being allowlisted.

Honest: no game internals; states come from the search's own solutions, noise is
the eval's own sticky model, "progress" is the wavefront distance (search-derived).
"""
...
def main() -> int:
    ...
    for b in range(n_batches):
        ...
        pend = [[] for _ in range(W)]      # (obs, action) buffer per worker
        alive = [True] * W
        last_d = list(d0)
        for j in range(args.recovery_window):
            ...
            for i in range(W):
                a = int(actions[idx[i]])
                if alive[i]:
                    pend[i].append((np.array(obs[i], dtype=np.int8, copy=True), a))
                acts[i] = bitmasks[a]
    ...
    print(f"[dars] DONE: {kept} recoveries, {len(obs_rows)} (obs,action) pairs "
          f"-> {out}", flush=True)
    return 0
