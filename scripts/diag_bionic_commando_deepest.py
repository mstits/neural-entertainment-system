"""Deepest-cell diagnostic for the Bionic Commando onboarding smoke (wave 3).
...
`lives` byte (discover_observables.py 3-probe protocol, receipt
runs/onboard_wave3/bionic_commando_observables.json). No disassembly, no
external RAM maps. (This profile's `y` key is a documented inert
placeholder — see configs/bionic_commando.yaml — so it is not read here.)
...
and reports:
  ...
  - the profile's `lives` (0x00C1) byte before/after
...
ARCHIVE = "runs/onboard_wave3/smoke_bionic_commando/archive.pkl"
LIVES_ADDR = 0x00C1
AREA_ADDR = 0x0097
HOLD_STEPS = 150
...
def main() -> None:
    ...
    for name, mask in ACTIONS.items():
        ...
        print(f"[hold {name:8s}] odo_x {x0}->{x1} (d{x1-x0:+d})  "
              ...
              f"area@0x97 {area0}->{area_end} seq={area_seq}  "
