...
`make demo GAME=mario` loads the latest checkpoint, plays one
deterministic (argmax) episode from the profile's start state, records
the rendered frames to `demos/<game>_iter<N>.gif`, and reports the
outcome (furthest stage + whether the game/level was cleared, via the
same Rust reward fn the trainer uses).

The "show me it works" command (thesis Phase 5). Works for both policy
families: tile-encoder games (SMB) use the MLP + RAM-decoded obs; pixel
games (Contra/Castlevania/Mega Man) use the CNN + 84x84 preprocessed obs.
...
from train_game import DEFAULT_ROMS, resolve_profile_path  # noqa: E402
from eval_game import find_latest_checkpoint  # noqa: E402
